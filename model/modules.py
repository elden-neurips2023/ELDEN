...
from model.dynamics_utils import forward_network
...
class ChannelMHAttention(nn.Module):
    def __init__(self, chan_shape, attention_dim, num_heads, num_queries, query_dim, num_keys, key_dim,
        ...
        for fc_dim in post_fc_dims:
            b = 1 / np.sqrt(fc_dim)
            self.post_fc_weights.append(nn.Parameter(torch.FloatTensor(*chan_shape, num_queries, fc_dim, out_dim).uniform_(-b, b)))
            self.post_fc_biases.append(nn.Parameter(torch.zeros(*chan_shape, num_queries, 1, out_dim)))
    ...
    def forward(self, q, k, return_attn=False, gumbel_select=False, tau=1.0, log_attn_mask=None):
        ...
        query = forward_network(q, self.query_weight, self.query_bias)  # (chan_shape, num_queries, bs, embed_dim)
        key = forward_network(k, self.key_weight, self.key_bias)        # (chan_shape, num_keys, bs, embed_dim)
        ...
        value = forward_network(k, self.value_weight, self.value_bias)  # (chan_shape, num_keys, bs, embed_dim)
